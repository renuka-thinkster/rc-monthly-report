...
// WA / EXPORT
// ══════════════════════════
function waText(){const p=getGPeriod();const s=MD_YEARLY.find(d=>d.m===p.short)||MD_YEARLY[MD_YEARLY.length-1];window.open('https://wa.me/?text='+encodeURIComponent(`*RC MIS — ${p.label}*\nSales: ₹${f(s.s)}\nFood Cost: ₹${f(s.fc)} (${(s.fc/s.s*100).toFixed(1)}%)\nNet Profit: ₹${f(s.np)} (${(s.np/s.s*100).toFixed(1)}%)\n\nYTD: ₹1,15,83,406 | FC%: 36.41% | NP%: 19.78%`));}
function waLink(){window.open('https://wa.me/?text=RC+MIS+'+encodeURIComponent(window.location.href));}
